    ...
    if(confirm('Të fshihet ky mësues? Caktimet e tij në klasa do të hiqen gjithashtu.')){
      ...
      Object.keys(DB.manualLocks).forEach(clsId=>{
        Object.keys(DB.manualLocks[clsId]).forEach(key=>{
          if(DB.manualLocks[clsId][key].teacherId===id) delete DB.manualLocks[clsId][key];
        });
      });
...
function getDisplayGrid(classId){
  ...
  Object.keys(clsLocks).forEach(key=>{
    ...
    if(grid[d]) grid[d][p] = {subjectId:lock.subjectId, teacherId:lock.teacherId, locked:true};
  });
  return grid;
}

/* Sets a manual lock for classId at day d, period p. If another class has a
   MANUAL lock on the same teacher at the same slot, this is rejected (the
   user must remove that lock first). If another class has that teacher
   there only from AUTO-generation (not locked), that other cell is freed up
   so the next "Gjenero/Rigjenero" can repair/refill it automatically. */
function applyManualLock(classId, d, p, subjectId, teacherId){
  for(const otherClassId in DB.manualLocks){
    ...
    if(other && other.teacherId===teacherId){
      alert(`Konflikt: ky mësues është kyçur manualisht te klasa ${otherClassId} në këtë orë. Hiqe atë kyçje së pari.`);
      return false;
    }
  }
  if(DB.timetable){
    ...
    if(prevCell && DB.timetable.teacherSchedule[prevCell.teacherId]){
      DB.timetable.teacherSchedule[prevCell.teacherId][d][p] = null;
    }
    DB.timetable.schedule[classId][d][p] = {subjectId, teacherId, locked:true};
    if(!DB.timetable.teacherSchedule[teacherId]) DB.timetable.teacherSchedule[teacherId] = emptyGrid();
    DB.timetable.teacherSchedule[teacherId][d][p] = classId;
  }
  if(!DB.manualLocks[classId]) DB.manualLocks[classId] = {};
  DB.manualLocks[classId][cellKey(d,p)] = {subjectId, teacherId};
  saveDB();
  return true;
}
...
function clearManualLock(classId,d,p){
  ...
  if(DB.timetable && DB.timetable.schedule[classId]){
    ...
    if(cell){
      if(DB.timetable.teacherSchedule[cell.teacherId]) DB.timetable.teacherSchedule[cell.teacherId][d][p] = null;
...
function openCellEditModal(classId, d, p){
  ...
  openModal(`${classId} · ${dayName}, Ora ${p+1}`, `
    ...
    <div class="field">
      <label>Mësuesi</label>
      <select name="teacherId" id="cellTeacherSelect"></select>
    </div>
  ...
  `, (fd)=>{
    ...
    const teacherId = fd.get('teacherId');
    if(!subjectId){
      clearManualLock(classId,d,p);
      toast('Kutia u liruar — do të plotësohet automatikisht.');
    }else{
      if(!teacherId){ toast('Zgjidh një mësues.'); return; }
      const ok = applyManualLock(classId,d,p,subjectId,teacherId);
      if(!ok) return;
  ...
  function populateTeacherOptions(subjectId){
    const sel = document.getElementById('cellTeacherSelect');
    ...
    let html = `<option value="">— zgjidh —</option>`;
    eligible.forEach(t=> html += `<option value="${t.id}" ${prevLock&&prevLock.teacherId===t.id?'selected':''}>${escapeHtml(t.name)}</option>`);
    if(others.length){
      html += `<optgroup label="Të tjerë (nuk e japin këtë lëndë)">`;
      others.forEach(t=> html += `<option value="${t.id}" ${prevLock&&prevLock.teacherId===t.id?'selected':''}>${escapeHtml(t.name)}</option>`);
      html += `</optgroup>`;
    }
    sel.innerHTML = html;
  }
  ...
  populateTeacherOptions(initialSubject);
  document.getElementById('cellSubjectSelect').addEventListener('change', (e)=> populateTeacherOptions(e.target.value || subjIds[0]));
}
  ...
  for(let p=0;p<periods;p++){
    ...
    for(let d=0; d<days.length; d++){
      ...
      if(cell){
        ...
        body += `<td class="tt-cell${lockedCls}${editableCls}" data-day="${d}" data-period="${p}">${pin}<span class="subj">${escapeHtml(subj?subj.name:'—')}</span><span class="teach">${escapeHtml(teach?teach.name:'')}</span></td>`;
      }else{
